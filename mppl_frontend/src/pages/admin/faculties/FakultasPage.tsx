...
export default function FakultasPage() {
  useDocumentTitle("Manajemen Master Fakultas");
  ...
  const fetchFakultasData = async () => {
    ...
    try {
      ...
      setFakultas(Array.isArray(result) ? result : []);
    } catch (err: any) {
      console.error(err);
      setErrorMsg("Gagal memuat master data fakultas dari server Laravel.");
    } finally {
      setIsFetchLoading(false);
    }
  };

  const handleOpenDeleteModal = (item: Fakultas) => {
  ...
  const handleDeleteSubmit = async () => {
    ...
    try {
      await api.delete(`/faculties/${selectedFakultas.id}`, {
        headers: { Authorization: `Bearer ${token}` },
      });

      setToastType("success");
      setToastMessages([
        `Fakultas ${selectedFakultas.nama_fakultas} berhasil dihapus secara permanen.`,
      ]);
  ...
  const filteredFakultas = fakultas.filter((item) => {
    const query = searchQuery.toLowerCase().trim();
    return (
      !query ||
      item.kode_fakultas.toLowerCase().includes(query) ||
      item.nama_fakultas.toLowerCase().includes(query) ||
      (item.deskripsi && item.deskripsi.toLowerCase().includes(query))
    );
  });

  const totalItems = filteredFakultas.length;
  const totalPages = Math.ceil(totalItems / perPage) || 1;
  const indexOfLastItem = currentPage * perPage;
  const indexOfFirstItem = indexOfLastItem - perPage;
  const currentDataSlice = filteredFakultas.slice(
    indexOfFirstItem,
    indexOfLastItem,
  );
              ...
              >
                ...
                <p className="text-xs sm:text-sm text-slate-500 leading-relaxed bg-slate-50 p-3.5 border border-slate-100 rounded-md">
                  ...
                  <strong className="text-slate-900 font-bold block mt-1">
                    {selectedFakultas?.nama_fakultas} (
                    {selectedFakultas?.kode_fakultas})
                  </strong>
                  Tindakan ini permanen dan berpotensi memutus relasi data prodi
                  yang terkait di database.
                </p>
