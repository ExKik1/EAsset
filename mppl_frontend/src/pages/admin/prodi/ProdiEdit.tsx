...
export default function ProdiEdit() {
  useDocumentTitle("Ubah Data Master Program Studi");
  ...
  useEffect(() => {
    if (token) {
      fetchFakultasOptions();
      if (id) fetchExistingProdi();
    }
  }, [token, id]);

  // Menutup dropdown otomatis jika pengguna mengklik area luar dropdown
  useEffect(() => {
  ...
  const fetchFakultasOptions = async () => {
    try {
      ...
      console.error("Gagal mengambil opsi master fakultas:", err);
    }
  };

  const fetchExistingProdi = async () => {
    try {
      ...
      setForm({
        fakultas_id: data.fakultas_id ? String(data.fakultas_id) : "",
      ...
      setToastType("danger");
      setToastMessages([
        "Gagal mengambil data detail program studi dari server.",
      ]);
      setTimeout(() => {
        navigate("/prodi");
      }, 1500);
  ...
  const handleUpdate = async (e: React.FormEvent) => {
    ...
    try {
      await api.put(`/prodi/${id}`, form, {
        headers: { Authorization: `Bearer ${token}` },
      });

      setToastType("success");
      setToastMessages([
        "Perubahan master data program studi berhasil disimpan!",
      ]);
      ...
      if (parsedMessages.length === 0) {
        parsedMessages = [
          "Terjadi galat saat memperbarui data master program studi.",
        ];
      }
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 bg-white p-4 sm:p-5 border border-info-border shadow-sm">
            <div>
              ...
              <p className="text-slate-500 text-xs sm:text-sm">
                Formulir pembaruan entitas master data program studi pada
