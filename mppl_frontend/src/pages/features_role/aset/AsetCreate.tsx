...
import { ArrowLeft, Save, PackagePlus, RefreshCw, RotateCcw } from "lucide-react";
...
export default function AssetCreate() {
  useDocumentTitle("Tambah Aset Baru");
  ...
  // Ambil opsi klasifikasi kategori dari server API
  useEffect(() => {
    const fetchKategoriOptions = async () => {
      try {
        ...
        console.error("Gagal memuat arsitektur kategori", err);
        setToastType("danger");
        setToastMessages(["Gagal memuat konfigurasi basis data master kategori."]);
      }
    };

    if (token) fetchKategoriOptions();
  }, [token]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setToastMessages(["Item log aset baru berhasil didaftarkan ke sistem!"]);
