...
interface KategoriAsetHeaderProps {
  isFetchLoading: boolean;
  onRefresh: () => void;
}

export default function KategoriAsetHeader({
  isFetchLoading,
  onRefresh,
}: KategoriAsetHeaderProps) {
  return (
    <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 bg-white p-4 sm:p-5 border border-info-border shadow-sm">
      <div>
        <div className="flex items-center gap-2 mb-1">
          <Tags className="w-5 h-5 text-[#0b2f9f]" />
        ...
        <p className="text-slate-500 text-xs sm:text-sm">
          Manajemen klasifikasi golongan, pengelompokan, dan jenis aset logistik
          organisasi.
        </p>
      </div>
